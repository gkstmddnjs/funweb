...
%>				
				<tr onclick="location.href='content.jsp?num=<%=bean.getNum()%>&pageNum=<%=pageNum%>'">
					...
					<%
						int wid = 0;//답변글에 대한 들여쓰기 정도 값 저장
			...
			<div id="page_control">
				<%
					if(count > 0){//db에 글이 존재 한다면
						...
						//전체 페이지수 구하기 조건 3항 연산자 사용 -> 조건식  ? 참: 거짓 
						//전체 페이지수 = 전체글개수 / 한페이지당 보여줄 글 개수 + (전체글수를 한페이지에 보여줄 글수로 나눈 나머지값)
						...
						//시작페이지 번호 구하기 공식 ↓
						//((현재 클릭한 페이지번호 / 한블럭에 보여줄 페이지 개수) - (현재 클릭한 페이지번호를 한블럭에 보여줄 페이지개수로 나눈 나머지값)) * 한블럭에 보여줄 페이지개수 + 1
						int startPage = ((currentPage / pageBlock) -(currentPage % pageBlock == 0 ? 1:0)) * pageBlock + 1;
						
						//끝페이지 번호 구하기
						//끝페이지 번호 = 시작페이지번호 + 한블럭에 보여줄 글개수 -1;
						int endPage = startPage + pageBlock - 1;
						
						//[이전] 시작페이지 번호가 한 블럭에 보여줄 페이지 개수보다 클때 
